...
def func_H2_industry_enduses(file_path_scen,file_path_ref, run_name_scen,run_name_ref, output_folder):
    ...
    work_folder = output_folder + 'INDHH2' + '/'
    ...
    for process in H2_cons_ref['name'].unique():
        ...
        H2_cons_ref = pd.merge(H2_cons_ref,label_match,how='inner',left_on='2',right_on='process')
        H2_cons_ref = H2_cons_ref.drop_duplicates()
        H2_cons_ref['surname'] = H2_cons_ref['name'].apply(lambda x: ''.join(x.split('-')[:2]) if x.count('-')==3 else ''.join(x.split('-')[0]))
        H2_cons_ref = H2_cons_ref[H2_cons_ref['surname'] == process]
        
        H2_cons_ref['enduse'] = H2_cons_ref['name'].apply(lambda x: ''.join(x.split('-')[2]) if x.count('-')==3 else ''.join(x.split('-')[1]))


        H2_cons_ref['group'] = H2_cons_ref['enduse'].str.cat(H2_cons_ref['3'].astype(str), sep='_')
        ...
        H2_cons_scen = var_Fin[var_Fin['1'] == 'INDHH2']
        ...
        H2_cons_scen['surname'] = H2_cons_scen['name'].apply(lambda x: ''.join(x.split('-')[:2]) if x.count('-')==3 else ''.join(x.split('-')[0]))
        H2_cons_scen = H2_cons_scen[H2_cons_scen['surname'] == process]
        
        H2_cons_scen['enduse'] = H2_cons_scen['name'].apply(lambda x: ''.join(x.split('-')[2]) if x.count('-')==3 else ''.join(x.split('-')[1]))

        H2_cons_scen['group'] = H2_cons_scen['enduse'].str.cat(H2_cons_scen['3'].astype(str), sep='_')
        ...
        pivot_df = H2_cons_ref.pivot(index='3', columns='enduse', values='MtH2')
        ...
        pivot_df = H2_cons_scen.pivot(index='3', columns='enduse', values='MtH2')
        column_sums = pivot_df.sum(skipna=True)
        sorted_columns = column_sums.sort_values(ascending=False)
        ...
        def plot_stacked_bars_with_differentiation(df1, df2):
            ...
            ax.set_title(f'H2 Consumption in {process}', fontsize=22)
            ...
            second_legend = ax.legend(handles=[empty_patch, hashed_patch], labels=['Reference', 'Scenario'],
                                    fontsize=14, fancybox=True, shadow=True, loc='lower right')
